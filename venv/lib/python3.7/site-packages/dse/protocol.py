...
class ResultMessage(_MessageType):
    ...
    # Names match type name in module scope. Most are imported from dse.cqltypes (except CUSTOM_TYPE)
...
class _ProtocolHandler(object):
    ...
    result decoding or type deserialization. Class definitions are passed to :class:`dse.cluster.Cluster`
    on initialization.
...
if HAVE_CYTHON:
    from dse.obj_parser import ListParser, LazyParser
...
if HAVE_CYTHON and HAVE_NUMPY:
    from dse.numpy_parser import NumpyParser
